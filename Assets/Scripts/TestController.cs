...
public abstract class TestController : MonoBehaviour
{
    ...
    protected ParticleBehaviour[,] columns;
    ...
    {
        GameObject simpleParticle = (GameObject)Resources.Load("Prefubs/" + resourceName);
        if (simpleParticle == null)
        {
            Debug.LogError(resourceName + " is not found!");
            return;
        }
        columns = new ParticleBehaviour[width, height];
        if (width == 0 || height == 0) return;
        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++)
            {
                GameObject newObj = Instantiate(simpleParticle);
                columns[i, j] = newObj.GetComponent<ParticleBehaviour>();
        ...
        ParticleBehaviour.waweSpeed = attenuation;
        ParticleBehaviour.amplitude = amplitude;
        ParticleBehaviour.mouseEnter = OnMouseEnter;
        ParticleBehaviour.mouseExit = OnMouseExit;
        idleWait = true;
    }
    ...
    {
        idleWait = false;
        ParticleBehaviour.isIdle = false;
